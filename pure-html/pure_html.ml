...
let rec fold ~tag ~txt ~comment value = function
  ...
  | Tag { name; attrs; children = Some c } ->
    c |> List.fold_left (fold ~tag ~txt ~comment) value |> tag name attrs
  | Txt t -> txt t value
...
module Hx = struct
  ...
  let disabled_elt fmt = string_attr "data-hx-disabled-elt" fmt
  ...
  let inherit_ fmt = string_attr "data-hx-inherit" fmt
